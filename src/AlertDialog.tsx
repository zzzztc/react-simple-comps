...
import { Dispose, renderElement, beforeDisposeGen } from './helper/dom';
import { getThemeColorCss } from './helper/themeHelper';
...
const AlertDialog = forwardRef<HTMLDivElement, Props>((props, ref) => {
  ...
  return (
    <StyledAlertDialog
      {...rest}
      ref={ref}
      className={clsx('rsc-alert-dialog', className, { mobile })}
      visible={visible}
      onClose={onClose}
      position="center"
      mask={mask}
    ...
    >
      {closable && <Icon type="rsc-icon-guanbi" className="close" onClick={onClose} />}
...
AlertDialog.displayName = 'RSC-AlertDialog';
...
const show = (props: StaticProps) => {
  ...
  const beforeDispose = beforeDisposeGen(container, '.rsc-popup', transitionDuration);
